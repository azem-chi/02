...
// ══════════════════════════════════════════
// FIX-GOOGLE-1: نضبط الـ flag فوراً قبل أي شيء
// onAuthStateChanged يُطلق قبل getRedirectResult
// ══════════════════════════════════════════
...
window.firebaseSignIn = async function() {
  ...
  try {
    ...
    // نضبط الـ flag أولاً قبل أي طلب — لأن redirect يعيد تحميل الصفحة
    ...
    // استخدم redirect دائماً على Android أو standalone — popup غير موثوق
    if (isStandalone || isMobileUA) {
      await signInWithRedirect(auth, provider);
      // الكود لن يصل هنا — الصفحة ستُعاد تحميلها
      return;
    }
    ...
    if (e.code !== 'auth/popup-closed-by-user' && e.code !== 'auth/cancelled-popup-request') {
      const msgs = {
        'auth/popup-blocked': '⚠️ المتصفح حجب النافذة المنبثقة — جرّب مرة أخرى',
        'auth/network-request-failed': '⚠️ خطأ في الشبكة — تحقق من الاتصال',
      };
      showMiniToast(msgs[e.code] || '⚠️ فشل تسجيل الدخول: ' + (e.code || e.message));
    }
  }
};
...
window.obFirebaseGoogleSignIn = async function() {
  ...
  try {
    ...
    if (e.code !== 'auth/popup-closed-by-user' && e.code !== 'auth/cancelled-popup-request') {
      const msgs = {
        'auth/popup-blocked': '⚠️ المتصفح حجب النافذة — جرّب مرة أخرى',
...
// ══════════════════════════════════════════
// FIX-GOOGLE-5: معالجة Redirect Result
// المشكلة السابقة: flags تُمسح داخل getRedirectResult قبل استخدامها
// الحل: نقرأ الـ flags أولاً، ثم نمسحها بعد المعالجة
// ══════════════════════════════════════════
getRedirectResult(auth).then(async result => {
  // نقرأ الـ flags قبل أي شيء
  const wasOb       = localStorage.getItem('azem_ob_redirect')       === '1';
  const wasSettings = localStorage.getItem('azem_settings_redirect') === '1';

  if (result && result.user) {
    // نمسح الـ flags بعد قراءتها ناجحاً
    localStorage.removeItem('azem_ob_redirect');
    localStorage.removeItem('azem_settings_redirect');
    ...
    if (wasOb) {
      ...
      window._obGoogleJustSignedIn = false;
      updateAuthUI(result.user);
      // إذا كان مستخدماً موجوداً أو له بيانات — أنهِ الـ onboarding
      ...
      updateAuthUI(result.user);
      showMiniToast('☁️ مرحباً ' + (result.user.displayName || '').split(' ')[0] + '!');
      // FIX: فتح الإعدادات تلقائياً بعد redirect ليرى المستخدم أنه سجّل دخوله
      setTimeout(() => {
      ...
      // redirect من مكان غير معروف — فقط حدّث الواجهة
      updateAuthUI(result.user);
      showMiniToast('☁️ مرحباً ' + (result.user.displayName || '').split(' ')[0] + '!');
    }
  } else {
    // لا يوجد redirect result
    window._obGoogleJustSignedIn = false;
    localStorage.removeItem('azem_ob_redirect');
    localStorage.removeItem('azem_settings_redirect');
  }
